...
def copy_preceding_redis_state(bookmark_name, folder_dir):
    """Copy redis_after.json from the preceding bookmark to redis_before.json of current bookmark"""
    preceding_bookmark = find_preceding_bookmark(bookmark_name, folder_dir)
...
def copy_specific_bookmark_redis_state(source_bookmark_arg, target_bookmark_name, target_session_dir):
    ...
    # Parse the source bookmark argument (may be "bookmark" or "folder:bookmark")
    source_folder_name, source_bookmark_name = parse_session_bookmark_arg(
        source_bookmark_arg)
    ...
    if source_folder_name:
        ...
        active_folders = get_all_active_sessions()
    ...
    print(
        f"📋 Copying Redis state from '{source_folder_name}:{source_bookmark_name}' to '{target_bookmark_name}'")

    # Ensure target bookmark directory exists
    target_dir = os.path.join(target_folder_dir, target_bookmark_name)
...
def copy_initial_redis_state(bookmark_name, folder_dir):
